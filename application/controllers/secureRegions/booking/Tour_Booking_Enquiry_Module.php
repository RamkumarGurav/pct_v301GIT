...
class Tour_Booking_Enquiry_Module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('administrator/booking/Tour_Booking_Enquiry_Model');
	...
	function index()
	{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/booking/Tour_Booking_Enquiry_Module/listings', $this->data);
		parent::get_footer();
	}
	...
	function listings()
	{
		...
		$data_count = $this->Tour_Booking_Enquiry_Model->get_tour_booking_enquiry($search);
		...
		$this->data['tour_booking_enquiry_data'] = $this->Tour_Booking_Enquiry_Model->get_tour_booking_enquiry($search);
	...
	function view($tour_booking_enquiry_id = "")
	{
		...
		if (empty($tour_booking_enquiry_id)) {
		...
		$this->data['tour_booking_enquiry_data'] = $this->Tour_Booking_Enquiry_Model->get_tour_booking_enquiry(array("tour_booking_enquiry_id" => $tour_booking_enquiry_id, "details" => 1));
		if (empty($tour_booking_enquiry_id)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
		...
		$this->data['tour_booking_enquiry_data'] = $this->data['tour_booking_enquiry_data'][0];
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/booking/Tour_Booking_Enquiry_Module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($tour_booking_enquiry_id = "")
	{
		...
		if (!empty($tour_booking_enquiry_id)) {
			$this->data['tour_booking_enquiry_data'] = $this->Tour_Booking_Enquiry_Model->get_tour_booking_enquiry(array("tour_booking_enquiry_id" => $tour_booking_enquiry_id, "details" => 1));
			if (empty($this->data['tour_booking_enquiry_data'])) {
			...
			$this->data['tour_booking_enquiry_data'] = $this->data['tour_booking_enquiry_data'][0];
			$tour_id = $this->data['tour_booking_enquiry_data']->tour_id;
			$this->data['tour_date_data'] = $this->Common_Model->getData(array('select' => '*', 'from' => 'tour_date', 'where' => "tour_id = $tour_id"));
		}


		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/booking/Tour_Booking_Enquiry_Module/edit', $this->data);
		parent::get_footer();
	}
	...
	function doEdit()
	{
		...
		$tour_booking_enquiry_id = $_POST['tour_booking_enquiry_id'];
		...
		if (!empty($tour_booking_enquiry_id)) {
			...
			$insertStatus = $this->Common_Model->update_operation(array('table' => 'tour_booking_enquiry', 'data' => $enter_data, 'condition' => "tour_booking_enquiry_id = $tour_booking_enquiry_id"));
			...
			$tour_booking_enquiry_id = $insertStatus = $this->Common_Model->add_operation(array('table' => 'tour_booking_enquiry', 'data' => $enter_data));
			if (!empty($insertStatus)) {
	...
	function doUpdateStatus()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_Model->update_operation(array('table' => "tour_booking_enquiry", 'data' => $update_data, 'condition' => "tour_booking_enquiry_id in ($ids)"));
				if ($response) {
	...
	function ajax_insert_tourget_tour_booking_enquiry()
	{
		...
		$enqui = $insertStatus = $this->Common_Model->add_operation(array('table' => 'tour_booking_enquiry', 'data' => $enter_data));
	...
	function export()
	{
		...
		$this->load->view('admin/booking/Tour_Booking_Enquiry_Module/export', $this->data);
	}
	...
	function pdf()
	{
		...
		$html = $this->load->view('admin/booking/Tour_Booking_Enquiry_Module/pdf', $this->data, true);
		//echo $html;
